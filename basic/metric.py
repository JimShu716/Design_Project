...
class DCGScorer(PrecisionScorer):
    ...
    def getDCG(self, sorted_labels):
        ...
        dcgPart = [(math.pow(2, rel) - 1) / math.log(index + 1, 2) for index, rel in
                   enumerate(sorted_labels[:self.k], 1)]
...
if __name__ == "__main__":
    ...
    # for scorer in [APScorer(0), APScorer(1), APScorer(2), APScorer(3), PrecisionScorer(1), PrecisionScorer(2),
    # PrecisionScorer(10), NDCGScorer(10), RRScorer(0)]: print scorer.name(), scorer.score(sorted_labels)
